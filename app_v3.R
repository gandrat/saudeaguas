...
load('output_data/doencas_dados_v3.rda')
...
#datas para o slider--------
datas_unicas_slider <- casos_municipios_mes %>%
                 ...
                 tabPanel("Mapa principal de prevalência",
                          ...
                          absolutePanel(id = "controls", class = "panel panel-default",
                                        ...
                                        pickerInput("map_doenca", "Selecione a Doença:",
                                                    choices = unique(casos_municipios_mes$doenca),
                          ...
                          sidebarLayout(
                            sidebarPanel(
                              ...
                              pickerInput("estado_select_rgi", "Estados:",
                                          ...
                                          selected = unique(casos_municipios_mes$nm_uf),
                                          multiple = TRUE),
                              ...
                              pickerInput("mun_select", "Por Município:",
                                          choices = NULL,
                                          options = list(`actions-box` = TRUE),
                                          selected = unique(casos_municipios_mes$nm_mun),
                                          multiple = TRUE)
                            ),
                            mainPanel(
                              h4("Casos"),
...
server <- function(input, output, session) {
  ...
  casos_mun_db <- reactive({
    req(input$doenca_rgi, input$rgint_select, input$rgi_select,input$rgi_select )
    casos_municipios_mes %>%group_by(data,estacao)%>%
      filter(doenca == input$doenca_rgi, 
             nm_rgint %in% input$rgint_select, 
             nm_rgi %in% input$rgi_select)%>%
      summarise(total_casos=sum(total_casos,na.rm=T),
                prev=mean(prev,na.rm=T))
  })
  ...
  #GRÁFICOS---------
  ##Gráfico de casos por data----------
  output$grafico_mun <- renderPlotly({
    g <- ggplot(casos_mun_db(), aes(x = data, y = total_casos)) +
      geom_col(position = "stack") +
      labs(title = "", x = "Data", y = "Nº de Casos", fill = "Região Imediata") +
      theme_minimal() +
      theme(legend.title = element_text(size = 12))
    ggplotly(g)
  })
  
  ##Gráfico de prevalencia por data--------------
  output$grafico_taxa_mun <- renderPlotly({
  ...
  output$grafico_estacao <- renderPlotly({
    ...
    g <- ggplot(data_plot, aes(x = estacao, y = total_casos, fill = estacao,
                               group = estacao)) +
  ...
  # MAPA----------------
    ...
    addHeatmap(data = casos_mun_db_map, lat = ~ lat, lng = ~ lon, 
               # intensity = ~(prev-min(prev))/(max(prev)-min(prev))*50000,
               intensity = ~prev*5000,
               radius=25,
               blur=20,
               minOpacity=0.01,
